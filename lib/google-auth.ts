...
function normalizePrivateKey(raw: string): string {
  let k = raw.trim()
  // Support base64-encoded full PEM
...
export async function getValues(range: string): Promise<string[][]> {
  const res = await sheetsFetch(`/values/${encodeURIComponent(range)}?valueRenderOption=UNFORMATTED_VALUE`)
  if (!res.ok) {
    const t = await res.text()
    throw new Error(`getValues ${range}: ${res.status} ${t.slice(0, 200)}`)
  }
  const data = (await res.json()) as { values?: string[][] }
  return data.values || []
}

export async function updateValues(range: string, values: (string | number)[][]): Promise<void> {
  ...
  if (!res.ok) {
    const t = await res.text()
    throw new Error(`updateValues ${range}: ${res.status} ${t.slice(0, 200)}`)
  }
}

export async function clearValues(range: string): Promise<void> {
  const res = await sheetsFetch(`/values/${encodeURIComponent(range)}:clear`, {
    method: 'POST',
    body: JSON.stringify({}),
  })
  if (!res.ok) {
    const t = await res.text()
    throw new Error(`clearValues ${range}: ${res.status} ${t.slice(0, 200)}`)
  }
}
