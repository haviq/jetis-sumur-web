<template>
  <div>
    ...
    <div v-else-if="!auth.user" class="max-w-md mx-auto card p-6">
      ...
      <p class="text-sm muted mt-1">Hanya perangkat padukuhan. Halaman ini tidak ditampilkan di menu publik.</p>
      ...
      <p class="text-xs muted mt-4">
        Default demo: <code>admin</code> / <code>admin2026</code> ·
        <code>padukuhan</code> / <code>padukuhan2026</code> ·
        <code>superadmin</code> / <code>superadmin2026</code>
      </p>
    </div>
    ...
    <div v-else>
      ...
      <p class="muted text-sm mt-1">Mode: {{ stats?.mode }} · Selamat datang, {{ auth.user.nama }}</p>
      ...
      <div class="grid gap-4 mt-6 lg:grid-cols-2">
        ...
        <div class="card p-5">
          ...
          <ul class="space-y-2 text-sm max-h-64 overflow-auto">
            <li v-for="l in stats?.recentLogs || []" :key="l.id" class="border-b pb-2" style="border-color: var(--border)">
